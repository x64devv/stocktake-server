    ...
    <div className="p-6 space-y-6">
      ...
      <Card>
        <CardBody className="p-0">
          {(lines?? []).length === 0 ? (
            <Empty message="No items counted yet." />
          ) : (
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {(lines?? []).map(line => (
                  <tr key={line.item_no} className={clsx('hover:bg-gray-50', selected.has(line.item_no) && 'bg-teal-50')}>
                    ...
                    <td className="px-4 py-3 text-gray-900">{line.counted_qty}</td>
                    <td className="px-4 py-3 text-gray-900">{line.theoretical_qty}</td>
                    <td className={clsx('px-4 py-3 font-medium', line.variance !== 0 ? 'text-red-600' : 'text-gray-600')}>
                      {line.variance > 0 ? '+' : ''}{line.variance}
                    </td>
                    <td className={clsx('px-4 py-3 text-xs font-medium', Math.abs(line.variance_pct) > 2 ? 'text-red-600' : 'text-gray-600')}>
